...
void jazzCommons::debug_config_put(const string key, const string val)
{
	...
	TEST_CASE("Basic tests for jzzINSTALL", "[basic]") {
		jCommons.debug_config_put("key01", "6.5.4.3");
		jCommons.debug_config_put("key02", "8.7");
		jCommons.debug_config_put("key03", "9");

		int	   tt_cfg_i = 54321;
		double tt_cfg_d = 123.456;
		string tt_cfg_s ("nothing");

		REQUIRE(!jCommons.get_config_key("key00", tt_cfg_i));
		REQUIRE(!jCommons.get_config_key("key00", tt_cfg_d));
		REQUIRE(!jCommons.get_config_key("key00", tt_cfg_s));

		REQUIRE((tt_cfg_i == 54321 && tt_cfg_d == 123.456 && tt_cfg_s.compare("nothing") == 0));

		REQUIRE(!jCommons.get_config_key("key01", tt_cfg_i));
		REQUIRE(!jCommons.get_config_key("key01", tt_cfg_d));
		REQUIRE( jCommons.get_config_key("key01", tt_cfg_s));

		REQUIRE((tt_cfg_i == 54321 && tt_cfg_d == 123.456 && tt_cfg_s.compare("6.5.4.3") == 0));

		REQUIRE(!jCommons.get_config_key("key02", tt_cfg_i));
		REQUIRE( jCommons.get_config_key("key02", tt_cfg_d));
		REQUIRE( jCommons.get_config_key("key02", tt_cfg_s));

		REQUIRE((tt_cfg_i == 54321 && tt_cfg_d == 8.7 && tt_cfg_s.compare("8.7") == 0));

		REQUIRE( jCommons.get_config_key("key03", tt_cfg_i));
		REQUIRE( jCommons.get_config_key("key03", tt_cfg_d));
		REQUIRE( jCommons.get_config_key("key03", tt_cfg_s));

		REQUIRE((tt_cfg_i == 9 && tt_cfg_d == 9.0 && tt_cfg_s.compare("9") == 0));

		jCommons.debug_config_put("key03", "");
